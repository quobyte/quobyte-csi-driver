...
| --- | --- |
| [`sanity/`](./sanity) | Quobyte's own checks of the driver. |
| [`k8s-storage-tests/`](./k8s-storage-tests) | Sets up what an upstream Kubernetes suite needs via the Quobyte API, runs the suite, then removes that setup again. |
...
`e2e-tests/` (`github.com/quobyte/quobyte-csi-driver/e2e-tests`).
...
```bash
e2e-tests/test_runner --list  # list all tests
e2e-tests/test_runner --sanity --list      # just list the sanity set
e2e-tests/test_runner --upstream --list      # just list the upstream test set

# Run all the tests
e2e-tests/test_runner <http://host:port> <host:port> <client-image-url>
# Run only the sanity tests, skipping the long upstream suites
e2e-tests/test_runner --sanity <http://host:port> <host:port> <client-image-url>
# Run only the upstream suites
e2e-tests/test_runner --sanity <http://host:port> <host:port> <client-image-url>

# Deploy the quobyte-k8s-resources submodule's charts as they are (nothing built locally)
e2e-tests/test_runner --source=chart-images <http://host:port> <host:port> <client-image-url>
...
```bash
e2e-tests/test_runner --list  # List all the tests

# re-run just the one that failed
TESTS='dynamic_provisioning/default' e2e-tests/test_runner <http://host:port> <host:port> <client-image-url>

# or every environment of one test package, by naming the package
TESTS='external_storage' e2e-tests/test_runner <http://host:port> <host:port> <client-image-url>

# several patterns at once; globs still work
TESTS='expansion volume_metrics shared_volume*/default' e2e-tests/test_runner <http://host:port> <host:port> <client-image-url>
```
...
Each subdirectory of [`sanity/`](./sanity) with an `env/` directory is one
test package:

```
sanity/dynamic_provisioning/
...
setup) or by adding a new `sanity/<name>/` directory with its own `env/` and `_test.go`
files.

## The upstream tests

Each subdirectory of [`k8s-storage-tests/`](./k8s-storage-tests) with an `env/` directory is one
test package that drives an upstream Kubernetes suite, and is run once per environment just like a
sanity test:

```
k8s-storage-tests/external_storage/
...
The suite itself is not ours -- `e2e-tests/e2e` downloads the released `e2e.test`/`ginkgo`
...
2. **run**: write the StorageClass to `$ARTIFACTS_DIR` and hand it to `e2e-tests/e2e` via
